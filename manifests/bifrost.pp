...
class infracloud::bifrost (
  ...
  file { '/etc/dnsmasq.conf.eth2.25':
    ...
    content  => template('infracloud/bifrost/dnsmasq.conf.eth2.25'),
    mode    => '0644',
    require => Exec['install bifrost'],
    notify  => Service['dnsmasq-eth2.25'],
  }

  file { '/etc/dnsmasq.d/bifrost.dhcp-hosts.d':
    ensure  => directory,
    recurse => true,
    source  => 'puppet:///modules/infracloud/bifrost.dhcp-hosts.d/',
    mode    => '0644',
    require => Exec['install bifrost'],
    notify  => Service['dnsmasq-eth2.25'],
  }
