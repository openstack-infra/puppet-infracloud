...
class infracloud::bifrost (
  ...
  file { '/etc/dnsmasq.conf.eth2.25':
    ensure  => present,
    source  => 'puppet:///modules/infracloud/dnsmasq.conf.eth2.25',
    mode    => '0644',
    require => Exec['install bifrost'],
    notify  => Service['dnsmasq-eth2.25'],
  }
  ...
  file { '/etc/dnsmasq.d/bifrost.dhcp-hosts.eth2.25.d':
    ensure => directory,
  }
  ...
  exec { 'set up static dhcp hosts':
    ...
    path        => '/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin',
    unless      => 'test "$(ls -A /etc/dnsmasq.d/bifrost.dhcp-hosts.d/)"', # onlyif directory is empty
    notify      => Service['dnsmasq-eth2.25'],
    require     => [
      File['/opt/stack/custom_playbooks/deploy-public-dnsmasq.yaml'],
      File['/opt/stack/custom_playbooks/templates/dhcp-host.j2'],
  ...
  service { 'dnsmasq-eth2.25':
    ensure     => running,
    provider   => 'base',
    binary     => '/usr/sbin/dnsmasq --conf-file=/etc/dnsmasq.conf.eth2.25',
    hasstatus  => false,
    hasrestart => false,
    ...
    # /etc/dnsmasq.conf.eth2.25.
    pattern    => 'dnsmasq --conf-file=/etc/dnsmasq.conf.eth2.25'
  }
}
