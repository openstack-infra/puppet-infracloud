...
class infracloud::controller(
  ...
  file { ['/etc/rabbitmq/ssl/private', '/etc/rabbitmq/ssl/certs']:
    ensure => directory,
    owner  => 'root',
    mode   => '0755',
  }
  ...
  infracloud::ssl { 'rabbitmq':
    ...
    cert_path    => "/etc/rabbitmq/ssl/certs/${controller_public_address}.pem",
  }
  ...
  class { '::rabbitmq':
    ...
    ssl                   => true,
    ssl_only              => true,
    ssl_cacert            => $ssl_chain_path,
    ssl_cert              => "/etc/rabbitmq/ssl/certs/${controller_public_address}.pem",
