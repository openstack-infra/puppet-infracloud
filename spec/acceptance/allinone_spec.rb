...
describe 'allinone', :if => os[:family] == 'ubuntu' do
  ...
  before :each do
    ...
    # Set before each test, since a networking refresh might reset the hostname
    # on some clouds.
  ...
  it 'should apply the controller with no errors' do
    ...
    shell('cat /etc/hosts')
    shell('cat /var/lib/dhcp/dhclient.leases')
    shell('cat /etc/resolv.conf')
    shell('cat /etc/dhcp/dhclient.conf')
    shell('cat /mnt/config/openstack/latest/*')
  ...
  credentials += ' OS_AUTH_URL=https://localhost:5000/v3'
  ...
  it 'should have keystone endpoints' do
    ...
    expect(result.stdout).to match(/localhost:5000/)
    expect(result.stdout).to match(/localhost:9696/)
    expect(result.stdout).to match(/localhost:9292/)
    expect(result.stdout).to match(/localhost:8774/)
    expect(result.exit_code).to eq(0)
  end
  ...
  it 'should be able to upload an image' do
    ...
    expect(result.exit_code).to eq(0)
    sleep(3) # command returns immediately but image needs time to upload
