...
iface {bridge_name} inet static
    ...
    # tear down if exists, and up
    subprocess.call(['ifconfig', bridge_name, 'down'])
    subprocess.call(['brctl', 'delbr', bridge_name])

    subprocess.call(['ip', 'link', 'set', bridge_name, 'up'])
...
def main(argv):
    ...
    if network_info:
        ...
        if bridge_name not in interfaces:
            configure_bridge(current_eth, vlan_id, network_info, argv[0])
